...
@app.post("/reload-rasa-model")
async def reload_rasa_model():
    """
    Triggers Rasa Core/NLU server to load the latest model from its storage path.
    """
    RASA_HOST = "http://rasa:5005"  # 'rasa' is the service name in docker-compose.yml
    RASA_RELOAD_ENDPOINT = f"{RASA_HOST}/model"

    try:
        # The Rasa API uses a PUT request to trigger model reload
        response = requests.put(RASA_RELOAD_ENDPOINT)
        response.raise_for_status() # Raise an HTTPError for bad responses (4xx or 5xx)

        return JSONResponse(
            status_code=200,
            content={"message": "Rasa model reload successful", "rasa_response": response.json()}
        )
    except requests.exceptions.RequestException as e:
        print(f"ERROR: Failed to connect to Rasa for reload: {e}")
        return JSONResponse(
            status_code=500,
            content={"message": "Failed to trigger Rasa model reload", "detail": str(e)}
        )
...
async def train_model_task(training_job_id: int, dataset_id: int, file_path: str, format: str):
    ...
    try:
        ...
        # ----------------------------------------------------------------------
        # 💡 NEW AUTOMATIC MODEL RELOAD LOGIC
        # ----------------------------------------------------------------------
        log_messages.append("🔄 Triggering Rasa model reload on the live server...")
        RASA_RELOAD_ENDPOINT = f"{RASA_URL}/model"
        try:
            # Use async aiohttp PUT request to command Rasa to load the new model
            async with aiohttp.ClientSession() as reload_session:
                async with reload_session.put(RASA_RELOAD_ENDPOINT) as resp:
                    # Rasa responds with 204 No Content or 200 on success
                    if resp.status in [200, 204]:
                        log_messages.append("✅ Rasa model reload successful. New model is now live!")
                    else:
                        error_text = await resp.text()
                        log_messages.append(f"⚠️ Rasa reload failed (Status {resp.status}). Response: {error_text[:100]}...")
        except Exception as reload_error:
            # This happens if the Rasa service (rasa container) is not reachable
            log_messages.append(f"❌ Failed to reach Rasa server for reload: {reload_error}")

        # Final patch to update the log with the result of the reload attempt
        async with aiohttp.ClientSession() as session:
            await session.patch(
                f"{NEXT_API_URL}/training-jobs/{training_job_id}",
                json={
